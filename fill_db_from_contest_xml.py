...
def fill_db_from_contest_xml(contest_xmls_dir, cursor, origin):
    ...
    for _, filename in files_walker.walk(contest_xmls_dir):
        contest_name = ejudge_get_contest_name(filename)
        contest_id = os.path.basename(filename)
        cursor.execute('UPDATE Contests SET name = "?" WHERE origin = "?" AND contest_id = ?', (contest_name, origin, contest_id))